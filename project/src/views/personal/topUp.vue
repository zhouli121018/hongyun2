<template>
    <div v-if="info != null">
        ...
        <div class="member_title">
            ...
            <p v-html="info.buydesc">金币仅用于查看预测数据，购买后不能提现和退款。平台不保证100%准确，但平台100%保证公平公正。</p>
        </div>
        <div class="xian"></div>
        ...
        <p class="pay_faild" style="padding-bottom:0.8rem;">充值失败联系微信:{{$store.getters.homeData.kfwecha}}</p>
        <div>{{zhifubao}}</div>
    </div>
</template>

<script>
import { getbuyjindoudesc, getalipayorderinfor } from '@/api'
export default {
    data() {
        return {
            list: [],
    ...
    methods: {
        ...
        //列表获取接口
        async getbuyjindoudesc() {
            const { data } = await getbuyjindoudesc({
